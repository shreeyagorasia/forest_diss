...
### Residual Moran's I vs. EN/XGBoost (2026-08-15) — `[SUPERSEDED 2026-08-18]`

**`[SUPERSEDED 2026-08-18]`**: k=8 nearest-neighbour method replaced project-wide by a
semivariogram-informed distance-band method — same change, same reasoning, as
`TEMP_rq2_attribution_results_2026-08-11.tex`'s equivalent section. **All numbers below are
stale.** Corrected numbers and re-derived conclusion follow; original k=8 table kept inline below
for provenance, not for citing.

Re-run 2026-08-18 with `compute_residual_morans_i()` (`models/growth_curve_attribution/
residual_spatial_autocorrelation_check.py`, semivariogram-informed distance-band weights, 999
permutations, seed=42, max_distance=5000m, widened to 10000m for cells that did not resolve within
5000m), on RQ3's own current-tier EN/XGBoost and GNNWR residuals (`local_y_max_difference` target),
all 3 sets, both cohorts. No new fitting — all from already-saved `predictions.csv`/
`test_predictions.csv` files.

| Set / cohort | EN (range) | XGB (range) | GNNWR (range) | GNNWR vs. best global |
|---|---:|---:|---:|---:|
| Set2 / 4survey | 0.1471 (1008 m) | 0.1476 (992 m) | 0.1001 (815 m) | -0.0470 (-32.0%) |
| Set2 / 6survey | no structure | -0.0002 (10000 m, p=0.053) | -0.0002 (10000 m, p=0.223) | n/a — see note |
| Set3 / 4survey | 0.1671 (1017 m) | 0.1443 (917 m) | 0.1086 (808 m) | -0.0357 (-24.7%) |
| Set3 / 6survey | no structure | no structure | no structure | n/a |
| Set4 / 4survey | 0.1454 (1065 m) | 0.1068 (1134 m) | 0.0815 (1252 m) | -0.0253 (-23.7%) |
| Set4 / 6survey | no structure | -0.0002 (10000 m, p=0.309) | -0.0002 (10000 m, p=0.134) | n/a — see note |

"no structure" = semivariogram flat even at the 5000m window (genuine null, not a small/zero
value). 6survey XGB/GNNWR values marked "10000m" only resolved after widening the search window —
`exceeds_window` status, not `resolved`; reported as provisional, not equivalent evidence to the
resolved 4survey column. p=0.001 (permutation floor) in every resolved 4survey cell.

**Revised pattern — 4survey conclusion strengthens, 6survey conclusion changes, not just shrinks:**

1. **On 4survey, GNNWR reduces residual clustering relative to the best global model in all 3
   sets, and proportionally more than the k=8 method showed** (-0.025 to -0.047, a 24-32% relative
   reduction against the smaller corrected baseline, vs. -0.012 to -0.020 / ~2-3% relative under
   k=8). Same conclusion as before — GNNWR corroborated by an independent diagnostic, not just
   R2 — now with a stronger effect size.
2. **On 6survey, the "GNNWR increases clustering, wrong direction" finding does not survive.**
   Under k=8, 6survey showed substantial clustering (0.55-0.65) that GNNWR made worse (+0.031 to
   +0.052). Under the corrected method, most 6survey cells show no resolvable spatial structure at
   all (Set3: none of the 3 models resolve even within 5000m). The cells that do resolve only do
   so after widening to 10000m, and land at I≈-0.0002, not significant (p=0.05-0.31). There is no
   detectable clustering for GNNWR to have gotten the direction wrong about. **This removes Moran's
   I as independent corroboration of GNNWR "failing" on 6survey** — the R2 collapse (near-zero,
   sign-unstable across seeds) stands on its own evidence, not doubly-confirmed as before. Still
   consistent with the sample-density explanation (47 vs. 231 compartments) — 6survey may simply
   be too spatially sparse for this diagnostic to resolve at all, a third independent symptom of
   the same limitation (alongside R2 instability and reseed sign-flips), not a second, separate
   failure mode.

**This still closes the bridge flagged in RQ2b's own (also corrected) Moran's I finding**
(`TEMP_rq2_attribution_results_2026-08-11.tex`, EN/XGBoost residual Moran's I on `mean_cr_residual`,
now 0.03-0.15, not 0.65-0.74): RQ3's GNNWR still reduces that same kind of unexplained spatial
structure on 4survey, proportionally more clearly than before. The 6survey half of the bridge is
now "uninformative," not "answered in the expected direction" — a materially different claim, not
a rounding change.

**Original k=8 table (2026-08-15), for provenance only, not for citing**: Set2/4survey EN
0.7032/XGB 0.7059/GNNWR 0.6829 (diff -0.0203); Set2/6survey EN 0.5964/XGB 0.6064/GNNWR 0.6354 (diff
+0.0390); Set3/4survey EN 0.7069/XGB 0.6958/GNNWR 0.6843 (diff -0.0115); Set3/6survey EN 0.6060/XGB
0.5721/GNNWR 0.6031 (diff +0.0310); Set4/4survey EN 0.7179/XGB 0.7085/GNNWR 0.6962 (diff -0.0123);
Set4/6survey EN 0.5996/XGB 0.6091/GNNWR 0.6517 (diff +0.0521). All p=0.001.
