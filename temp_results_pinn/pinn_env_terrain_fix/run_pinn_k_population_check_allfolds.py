# PINN-k version of run_ymax_population_check_allfolds.py -- same pooling idea, but for PINN-k
# (both y_max and k are personalized here, plain PINN only personalizes y_max). Saves blk/cpmt
# alongside identification/Age/y_max_pred/k_pred, so all 5 folds' test-set predictions can be
# pooled into one full-forest map, matching what the plain-PINN map already does.
#
# Fold 0 already exists (temp_results_pinn/outputs/example_curve/test_set_predictions.csv,
# no blk/cpmt saved) -- this script covers folds 1-4, and could re-do fold 0 too if needed.
#
# Run (any fold): PYTHONPATH=. python temp_results_pinn/pinn_env_terrain_fix/run_pinn_k_population_check_allfolds.py --fold-index 1
#
# Saves to
...
OUTPUT_DIR = Path(__file__).resolve().parents[1] / "outputs" / "CORRECTED_2026-08-23_mechanism_checks" / "pinn_k_population_check"
...
def main():
    ...
    print("\nTraining fixed PINN-k (y_max and k both personalized), production settings...")
    model, _, history = fit(
    ...
    test_df["population_k"] = cr_params["k"]

    test_df[
        ["identification", "blk", "cpmt", "Age", "y_max_pred", "k_pred", "population_y_max", "population_k", "fold_index"]
