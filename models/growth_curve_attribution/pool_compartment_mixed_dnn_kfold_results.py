# Purpose: same as pool_simple_dnn_kfold_results.py, but for compartment_mixed_dnn_check.py's
...
from models.growth_curve_attribution.compartment_mixed_dnn_check import OUTPUT_DIR
...
def load_fold_predictions(cohort: str, scope: str, k_folds: int = DEFAULT_K_FOLDS) -> pd.DataFrame:
    ...
    for fold in range(k_folds):
        csv_path = OUTPUT_DIR / f"compartment_mixed_dnn_{scope}_{cohort}_fold{fold}of{k_folds}_test_predictions.csv"
...
def main():
    ...
    output_path = OUTPUT_DIR / f"compartment_mixed_dnn_{args.scope}_{args.cohort}_kfold_pooled_summary.csv"
