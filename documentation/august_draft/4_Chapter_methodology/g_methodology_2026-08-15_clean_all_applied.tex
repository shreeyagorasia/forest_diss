...
question's own Set1--5 (Table~\ref{tab:feature-sets-composition-rq1}). Three branches follow, one
...
\paragraph*{Evaluation Framework and Data Splits}. As established in \S\ref{sec:data-cohorts}, 4survey is the primary cohort throughout; 6survey supplies the temporal split and, where sample size allows, a secondary spatial check. For RQ1, each row in the dataset represents a single plot in a given survey year. For RQ2 and RQ3, data are aggregated so each row represents a single plot (Table~\ref{tab:rq-framework}). Model performance is evaluated using four distinct data-splitting strategies.
...
\bottomrule
\end{tabular}
\caption{The four-step screening protocol applied to build each research question's Set1--Set5
...
Three standard supervised models are fit on all RQ1 feature sets (baseline and Sets 1–5) using identical splits.
...
% window (roughly 15+ years, per the age-floor stated in \S\ref{sec:data-cohorts}) highlighted to
...
Feature importance and SHAP values provide post-hoc interpretability, compared against the mixed-effects coefficients for cross-model convergence (\S\ref{sec:eval-diagnostics}).
...
\item \textbf{Feature-set membership is fixed once, not re-derived per fold.} Set1--5 (\S\ref{sec:feature-final}) are ranked and VIF-screened once on the full training partition, not separately per cross-validation fold. A fold-specific ranking would better separate selection variance from model variance, but \textbf{was not used because it would let each of the five folds train on a different feature set}, breaking the direct model-family comparison every research question in this chapter relies on.
