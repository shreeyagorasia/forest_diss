%%% CLEAN DRAFT — all proposed edits from f_methodology_2026-08-14_2235_claude_rewrite.tex applied.
%
% What this file is: the "everything accepted" version. Every \ai{} note in file f (40 of them —
% typo/grammar fixes, itemize->table conversions, duplicate-paragraph removals, the results-
% leakage moves, the factual XGBoost correction, and the two new figure proposals) has been
% applied directly; there is no markup left to read here, just the resulting prose/tables/figures.
%
% How to use this alongside f: read them side by side. Anywhere this file's wording differs from
% f's original (black) text, that's one of the 40 applied changes. If you decide you don't want a
% specific one, find the matching \ai{} note in f (they're easy to locate — same paragraph, same
% topic) to see exactly what the original said and why the change was proposed, then either revert
% that passage here by hand or tell me which ones to revert and I'll do it.
%
% Not applied here (flagged in f as "your call", not given as a concrete instruction, so not
% counted among the 40): the RQ1 opening paragraph's stakeholder-investment aside, and
% Table~\ref{tab:feature-set-purpose}'s "Purpose" column prose-to-phrase tightening. Both are still
% open if you want to claw back more space later.
%
% Two things this file does NOT yet resolve, because they need your input, not an editorial call:
% (1) the XGBoost paragraph in \S\ref{sec:rq1-models} states RQ1's refit as sensitivity-check-only
%     and RQ2b's as adopted in production -- accurate today, but you said the RQ1 resolution is
%     still open; revisit that sentence once it's decided.
% (2) the two new figures (age/CR-linearity in \S\ref{sec:rq1-models}, and the relocated pipeline
%     overview in \S1) are still unrendered placeholders -- their captions are real, final text,
%     but the artwork itself doesn't exist yet.
%%%
...
% (\fc, \ct, \app, \sr, \fig are already defined below in this file)
...
cohorts feed the four data-splitting strategies
...
No branch currently produces a rendered coefficient or residual
map.}
...
Temporal (secondary) & Whole survey years assigned to train/val/test: 4survey trains 2008+2012, validates 2021, tests 2023; 6survey trains 2002--2012, validates 2021, tests 2023 \citep{lynch2025Digital_C4_NN}. & Tests generalisation across time (RQ1 only; RQ2/RQ3 have no temporal axis). \\
\bottomrule
\end{tabular}
...
RQ3 & \ct{RQ3's Set4 total column count -- pull from the actual feature manifest, don't guess} &
Elastic Net, XGBoost, GNNWR & Secondary & Primary & Which conditions align with
...
Feature importance and SHAP values provide post-hoc interpretability, compared against the mixed-effects coefficients for cross-model convergence.
...
Limitations specific to one model or target are stated beside that method (the CR functional-form assumption, the physics-loss misspecification risk, RQ3's asymptote-only target scope). This section covers the remaining cross-cutting limitations, using the "limited because X, retained because Y" pattern throughout rather than a general disclaimer.

\begin{itemize}
\setlength\itemsep{2pt}
\item \textbf{The spatial buffer is set by convention, not a measured range.} The 60\,m buffer is set from typical plot spacing, not from the residual's own measured spatial-autocorrelation range. A semivariogram-based buffer could tighten or loosen this bound, but \textbf{was not used to set the buffer itself because the semivariogram is computed from held-out residuals \emph{after} a split already exists} -- a circular basis for choosing that same split's buffer. Compartment-level holdout remains the primary leakage control; the buffer is a secondary refinement on top of it.
\item \textbf{Feature-set membership is fixed once, not re-derived per fold.} Set1--4 are ranked and VIF-screened once on the full training partition, not separately per cross-validation fold. A fold-specific ranking would better separate selection variance from model variance, but \textbf{was not used because it would let each of the five folds train on a different feature set}, breaking the direct model-family comparison every research question in this chapter relies on.
