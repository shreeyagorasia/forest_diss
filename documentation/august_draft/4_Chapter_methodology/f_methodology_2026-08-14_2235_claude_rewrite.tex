...
% Editing macros — all six (\fc, \ct, \app, \sr, \fig, \ai) are already defined below in this file
...
\ai{This paragraph's first sentence re-explains the 4survey/6survey cohort roles, which Chapter~\ref{ch:data} \S\ref{sec:data-cohorts} already states in full (the nested-cohort relationship, why 4survey is primary, why 6survey serves the temporal question). DELETE the first sentence and REPLACE with: "As established in \S\ref{sec:data-cohorts}, 4survey is the primary cohort throughout; 6survey supplies the temporal split and, where sample size allows, a secondary spatial check." Keep the rest of the paragraph (RQ1 vs RQ2/RQ3 row unit, and the lead-in to the four splits) as-is.}
...
% Set1-5) -- see the \ai{} note at that location above.
