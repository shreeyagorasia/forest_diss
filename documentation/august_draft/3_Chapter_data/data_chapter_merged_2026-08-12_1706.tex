%=======================================================================================
% DATA CHAPTER -- 2026-08-12, 17:06.
%
% Supersedes documentation/august_draft/3_Chapter_data/data_chapter_merged_2026-08-12.tex (no
% time-stamp) for one reason only: every table below is now a PLACEHOLDER, matching the figures'
% own convention (planned columns/values given as a comment above a \fbox placeholder, not a
% finished \tabular), instead of being fully typeset already. No prose, numbers, or verification
% status changed from that file -- only the table presentation. The earlier file is left on disk
% for reference, not deleted.
%
% Every number below was re-verified directly against data_processing/clean_master_data.py
...
% TABLE PLACEHOLDER (cleaning funnel)
% Planned columns: Stage | Four-survey rows | Four-survey plots | Six-survey rows | Six-survey plots
% Planned rows (verified 2026-08-12, live re-execution of
% data_processing/clean_master_data.py::clean_cohort against the raw GeoPackage):
...
\begin{table}[!htbp]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2.5cm}TABLE PLACEHOLDER --- cleaning funnel, see
comment above source for planned columns and values\vspace{2.5cm}}}
...
% TABLE PLACEHOLDER (cohort comparison)
% Planned columns: Characteristic | Four-survey cohort | Six-survey cohort
% Planned rows:
...
%   Compartments (cpmt) covered            296 | 48
%   Role                          Main analysis | Sensitivity check
\begin{table}[!htbp]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2.5cm}TABLE PLACEHOLDER --- cohort comparison, see
comment above source for planned columns and values\vspace{2.5cm}}}
...
\item \textbf{Canopy cover} (\texttt{CanopyCover}), an ALS-derived fraction (0--1) of canopy cover
  recorded per plot per survey. \fc{\texttt{CanopyCover} and the height target come from the same
  GeoPackage row -- the same ALS flight and the same area-based-analysis pipeline, for the same
  plot and survey year -- unlike the terrain/wind/climate/soil variables in
  \S\ref{sec:data-environmental}, which are matched in from entirely independent sources. This is
  a reverse-causation question, not an algebraic leak: canopy closure could be a consequence of
  prior growth, a partial independent driver of it (via planting density, stocking or thinning
  regime), or both, and the shared-provenance fact alone does not distinguish these.
  \texttt{CanopyCover} is nonetheless included as one of four baseline features in every
  environmental feature set used across all three research questions
  (\texttt{documentation/methodlogy\_env\_setpick.md}, \S1.6/\S2.11). This is addressed
  empirically rather than by assumption: a baseline-only ablation
  (\texttt{TEMP\_results/TEMP\_rq2\_attribution\_results\_2026-08-11.tex}, "CanopyCover
  reverse-causation check," added 2026-08-13) fits RQ2's attribution models on the baseline
  columns alone and compares against the full environmental sets. This Data chapter states the
  shared-provenance fact only; the ablation's outcome and its consequences for how
  \texttt{CanopyCover} is framed in the environmental feature sets belong in
  Chapter~\ref{ch:methodology}'s feature-set justification and the Results chapter, not here --
  verify those two sections actually carry this through once drafted.}
...
% TABLE PLACEHOLDER (external data sources)
% Planned columns: Data family | Source | Resolution | Temporal character | Main purpose |
%                  Plots per grid cell (avg.)
% Planned rows (resolution-vs-plot-count figures computed 2026-08-12 directly from plot
% coordinates in data/processed/environmental/plot_environmental_features.parquet, 71,766 plots;
% all other cells verified 2026-08-10 against documentation/methodlogy_env_setpick.md):
%   Forest plots           | Forest Research         | ~20m x 20m polygons  | Six survey years, 2002-2023            | Response variable, stand fields | -- (native unit)
%   Terrain                | OS Terrain 50 DTM       | 50m grid             | Static, single current DTM             | Elevation, slope, aspect, exposure | ~4.3
%   Soil (CEH categorical) | CEH natural capital     | 50m grid             | Static                                 | Pedotope, drainage, texture class | ~4.3
%   Soil pH                | SoilGrids v2.0, ISRIC   | 250m grid            | Static, topsoil (0-5cm)                | Soil acidity screening | ~53
%   Wind climatology       | Global Wind Atlas       | National raster grid| Long-term climatology, static          | General wind exposure | -- (1,816-1,824 distinct values across 71,766 plots, varies by height/variable)
%   Observed wind          | MIDAS Open stations     | 3 station points     | Survey-interval summaries, 2002-2023   | Temporal wind/storm conditions | -- (point data, not gridded)
%   Climate (long-term)    | CHELSA v2.1 BIOCLIM+    | 1km grid             | Static, 1981-2010 climatology          | GDD, precipitation | ~482
%   Climate (survey-matched)| HadUK-Grid, CEDA       | 1km grid             | Time-varying, cohort-year-averaged     | Temperature, ground frost | ~482
%   Spatial position       | OS Open Roads/Rivers; GPKG polygons | Vector distance calc. | Static | Edge/context effects | -- (distance-based, not gridded)
% Footnote to keep in caption: ERA5-Land temperature investigated (8 distinct values across all
% 71,766 plots, coarser than HadUK-Grid's 149) and not retained in any final feature set.
\begin{table}[!htbp]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2.5cm}TABLE PLACEHOLDER --- external data sources,
see comment above source for planned columns and values\vspace{2.5cm}}}
