...
Deep neural networks (DNNs) learn nonlinear interactions among multiple input variables through layered representations. A multi-layer perceptron optimises its weights through backpropagation to minimise a specified training loss. However, unconstrained empirical networks do not inherently preserve theoretical growth properties, such as monotonic height accumulation or bounded asymptotes, \cite{karatepe2022Total_C4_NN}. Without being explicitly programmed with a growth function, neural networks can still capture the underlying sigmoid curve, but could fail to maintain biological constraints across all ages. 
