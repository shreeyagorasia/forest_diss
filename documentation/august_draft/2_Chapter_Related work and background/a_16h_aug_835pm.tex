...
Top height reflects upper-canopy development and serves as the primary empirical indicator of forest site productivity. In British forestry, the height--age relationship underpins conventional General Yield Class (GYC) systems \citep{edwards1981Yield_C1_SS,ForestResearch2026Sitka_C1_SS}. GYC defines the maximum expected mean annual increment in cumulative stemwood timber volume ($\mathrm{m^3\,ha^{-1}\,yr^{-1}}$) under standard management prescriptions. For Sitka spruce, GYC is calculated directly from top height and stand age:
\begin{equation}
...
Global parametric and tree-based models assume that environmental relationships remain stationary across space. Geographically weighted regression (GWR) relaxes this assumption by fitting localized linear regressions via spatial distance-decay kernels \citep{brunsdon1996Geographically_C6_GWR}. Geographically and Neurally Weighted Regression (GNNWR) extends this framework by coupling a global OLS base model with a neural weighting network that learns continuous, distance-based parameter adjustments without requiring predefined bandwidths \citep{du2020Geographically_C6_GWR,yin2024GNNWR_C6_GWR}. In this context, Elastic Net and XGBoost capture global linear and non-linear associations, while GNNWR evaluates whether environmental relationships with growth departure exhibit spatial non-stationarity across the forest estate.
