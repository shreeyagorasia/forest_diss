...
British forestry management decisions are highly based on this height-age relationship, for estimations such as General Yield Class (GYC), a measure of site productivity \cite{edwards1981Yield_C1_SS,ForestResearch2026Sitka_C1_SS}. GYC defines the maximum expected mean annual increment in cumulative stemwood timber volume ($\mathrm{m^3\,ha^{-1}\,yr^{-1}}$). For Sitka spruce, GYC is calculated directly from top height and stand age:
\begin{equation}
...
The Chapman--Richards (CR) equation is widely adopted in forestry to represent height and yield trajectories through a flexible sigmoidal formulation \cite{richards1959Flexible_C2_CR,pommerening2015Methods_C2_CR,manso2021Dynamic_C2_CR}. In this context, the model is parameterised as:
\begin{equation}
...
XGBoost relaxes linearity by fitting sequential decision trees to capture non-linear thresholds and feature interactions, with TreeSHAP used post hoc to quantify feature contributions \citep{lundberg2017Unified_C7_XAI}.

To account for spatial non-stationarity (where the influence of environmental drivers shifts across the landscape), local regression models allow coefficients to vary by location. Where standard Geographically Weighted Regression (GWR) relies on fixed distance-decay kernels and bandwidths \citep{brunsdon1996Geographically_C6_GWR}, GNNWR uses a neural network to learn dynamic spatial weighting functions \citep{du2020Geographically_C6_GWR}. This preserves transparent local parameters while capturing complex regional variation, albeit at the cost of high pairwise distance computation and memory demand on large datasets.
