...
\item \textbf{A narrower, untested tuning gap -- still open.} XGBoost's hyperparameters were
grid-searched (27 configurations: n\_estimators, max\_depth, learning\_rate). The DNN already
trains with Adam (an adaptive, momentum-based optimiser) and a scheduler that halves the learning
rate on plateau -- this is not a single static learning rate. What was not searched is the
specific numeric settings of that setup: the initial learning rate, weight decay strength, and
scheduler factor/patience were fixed constants, never varied across candidate values the way
XGBoost's three hyperparameters were. Part of XGBoost's edge may still come from this narrower
gap. Untested, a real but smaller limitation than "the DNN used one fixed learning rate" would
imply.
\end{itemize}

Bottom line: the gap is real and consistent, but not fully explained. Three candidates are ruled
out, one is narrowed to "present but not decisive," and one remains untested.
