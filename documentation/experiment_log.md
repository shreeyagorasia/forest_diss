...
| `dnn_noenv_design1_smoketest` | 2026-07-15 | `temporal_split`, Design 1 | same as above | n/a (no physics term) | dnn_noenv | both | superseded (2-epoch sanity check only, not a real result — re-run to verify the new `elapsed_seconds` tracking column) | `outputs/dnn_noenv/<cohort>/` | Pipeline verified working; 4survey R²=0.4633 (best epoch was epoch 1 in both the 2- and earlier 5-epoch check, so metrics unchanged); not meaningful yet — real run needs full `--max-epochs` on SLURM |
| `pinn_noenv_design1_routeA_smoketest` | 2026-07-15 | `temporal_split`, Design 1 | same as above | Route A: plot_level CR fit (`outputs/chapman_richards/<cohort>/params.json`) | pinn_noenv | both | superseded (2-epoch sanity check only — re-run to verify the new `elapsed_seconds` tracking column) | `outputs/pinn_noenv/<cohort>/` | Pipeline verified working, both physics terms produce nonzero, non-dominating loss (physics_loss≈0.0093, trajectory_loss≈0.0198 @ epoch 2); 4survey R²=0.4032; not a real result yet |
