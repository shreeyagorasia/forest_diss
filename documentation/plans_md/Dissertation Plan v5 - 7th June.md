...
**This is a living plan, not a fixed contract.** Specific data sources, methods, deliverables, and even chapter structure below are the current best guess, made with the information available at the time -- not commitments to defend unchanged. Several have already been revised after real evidence contradicted them (e.g. HadUK-Grid's assumed "\~2-3 cells" and James Hutton's assumed "\~3-5 polygons," both disproven by actual extraction; see `documentation/experiment_log.md` and the environmental-data notebooks for the full record). Treat any specific claim here as a hypothesis to check against real data/analysis, not settled fact -- if evidence points to a different variable, method, or deliverable being more suitable, the plan should change to match the evidence, not the other way around.

The most recent example (28-29 July 2026): this plan's own "resolved" decision to use `Top_Height99` as the main response, and to keep `yldc` as a candidate feature, were both overturned by real evidence found during routine review -- `Top_Height99` was retired in favour of raw `elev_percentile_95th`, and `yldc` was removed as a feature everywhere after a real held-out ablation showed it hurts generalisation in every model tested (not from a theoretical concern -- see `documentation/progress_notes.md`'s "Systematic rebuild" entry for the full reasoning and numbers). Every `Top_Height99` reference below this point describes the plan as it stood before that decision -- read it as historical context for how the target was originally chosen, not as the current target.
...
**3.3 CR residuals** The primary, dataset-relative residuals use CR parameters optimised on the training split via least squares and applied to all plot-timestamp pairs. Box plots check whether residuals are centred near zero at each timestamp; a histogram of per-plot mean residuals \$\bar{\varepsilon}\_i\$ checks for systematic under-performance. Moran's I on \$\bar{\varepsilon}\_i\$ tests whether anomalies cluster spatially.

As a sensitivity benchmark, investigate residuals against the Sitka spruce top-height/GYC curve encoded by the supplied inventory's species lookup fields. The dataset's rows whose separate lookup field `Species == "SS"` consistently carry `p1=14.420680`, `p2=1.473882`, `p3=-1.142290`, `p4=0.035067`, and `p5=1.923684`; rearranging the supplied GYC equation gives

\[
TH(t;GYC)=(12.136100+1.473882\,GYC)\left(1-e^{-0.035067t}\right)^{1.923684}.
\]

This is a Chapman-Richards curve with \(k=0.035067\), \(p=1.923684\), and a GYC-dependent asymptote \(y_{\max}=12.136100+1.473882\,GYC\). There is no universal "typical Sitka spruce" parameter triplet: published models normally use site-index or yield-class families because site, provenance and management change the trajectory. The cleaned data contain `yldc`, a real externally sourced Forestry Commission inventory classification rather than a value calculated from the current survey's height and age. It is complete and **temporally constant within every plot, but not spatially constant across the filtered study area**. After the actual modelling filters, the four-survey cohort contains all 12 even classes from YC2--24 (58,112 plots), while the six-survey cohort contains 11 classes from YC2--24 (13,769 plots); YC24 is the mode in both. Therefore, a useful sensitivity reference can assign each plot the lookup curve corresponding to its documented `yldc`, rather than imposing one yield class on the entire forest. This use is benchmarking/stratification, not predictive-feature use; `yldc` remains excluded from the ML models because held-out ablations showed that it harms generalisation.

For each plot \(i\), the dataset-encoded calculation would use \(y_{\max,i}^{TH}=12.136100+1.473882\,yldc_i\), \(k=0.035067\), and \(p=1.923684\). Because this dissertation's response is raw \(P95\), while the inventory defines \(TH=1.10\times P95\), compare on the response scale using \(y_{\max,i}^{P95}=y_{\max,i}^{TH}/1.10\), with unchanged \(k\) and \(p\). For example, YC18 gives \((y_{\max}^{TH},k,p)=(38.665976,0.035067,1.923684)\), or \((35.150887,0.035067,1.923684)\) on the raw-P95 scale; YC24 gives \((47.509268,0.035067,1.923684)\), or \((43.190244,0.035067,1.923684)\) on the raw-P95 scale. Keep these lookup-curve residuals secondary unless the coefficient provenance is confirmed with Forest Research/the data provider: the exact numbers are present in the supplied data and attributed in the project notes to the Forest Research ALS inventory method, but the primary manual containing this coefficient table has not yet been independently located. In addition, `yldc` may itself originate from an earlier inventory assessment, and top height is not identical to raw LiDAR P95.
...
**Lookup-reference experiment (secondary robustness check, conditional on provenance confirmation).** Compare three residual definitions on exactly the same held-out plots: (A) fitted global CR; (B) the plot-specific dataset-encoded Sitka curve selected by the plot's documented `yldc`; and (C) a deliberately simpler cohort-level lookup curve selected before evaluation (YC18 for the main four-survey cohort and YC24 for the six-survey sensitivity cohort). Report age-banded bias/RMSE, observed-versus-predicted plots by documented YC, agreement between persistent over-/under-performer classifications, and Moran's I for each residual definition. Within each yield class, test whether terrain/wind still explains the lookup-reference residuals; this asks whether plots systematically depart from the growth expected for their recorded productivity class. Do not call this an independent published standard until the coefficient source is confirmed. Do not use `GYCspec95` to assign curves because it is calculated from the current height and age (target leakage), and do not re-fit \(k\) or \(p\) in the primary lookup test. An optional diagnostic may fit only a single multiplicative calibration factor on the training set to test whether the Top Height-to-P95 scale conversion is systematically offset.
