...
\end{equation}
\limit{In the auxiliary-networks-only implementation, $y_{\max}^{(i)}$ and $k^{(i)}$ condition the physics and trajectory loss terms but do not pass directly into the prediction trunk; the prediction remains the trunk's own output. See Section~\ref{sec:rq1-pinn-limitation}.
%TODO: flagged for a decision, not resolved here -- this note describes the ORIGINAL PINN
% forward pass. A later session (documentation/handover_2026-08-21.md) built and verified a
% fixed forward pass (prediction = CR(y_max_i, k_i, age) + trunk residual) and its 4-survey
% numbers are now the ones reported in Table~\ref{tab:rq-framework}'s parallel Results table --
% but this Methodology section still only describes the original, unfixed architecture. Needs
% a decision: describe both (original limitation, then the fix actually used for headline
% numbers), or update this section to describe the fixed architecture as primary with the
% original noted as a limitation of the wider PINN literature/earlier iteration.
}
