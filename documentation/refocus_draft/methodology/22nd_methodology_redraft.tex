...
% Merged with the Intro's plain-language objectives table (2026-08-22, user decision) --
% Intro keeps the vague/no-notation version for a first-time reader; this is the detailed
% version, with real notation, meant to be read after Related Work has already introduced
% GNNWR/LMM/PINN in general terms. Replaces the old tab:rq-framework alone.
\begin{table}[!ht]
\centering
\footnotesize
...
Q3 & Plot--survey row; top height itself ($H_{it}$), and row-level error reduction against the
...
Four feature sets test whether broader environmental information improves model performance. Sets are constructed separately for Q1's mean curve-departure target, Q2's asymptotic-departure target, and Q3's top-height target. Candidate variables with missing coverage or deterministic duplication are removed first. Near-identical variables with Spearman $|\rho|\geq0.95$ are reduced to one, preferring directly measured variables. Remaining candidates are ranked using the mean rank from Spearman $|\rho|$, permutation importance and drop-column ablation, using a single spatial-block split. Variables with variance inflation factor greater than 5 are removed iteratively \cite{obrien2007Caution_C5_EVAL}. Baseline variables are protected from removal but remain in the VIF calculation.

This ranking split is a different partition from the five-fold spatial-block split used for final evaluation (Section~\ref{sec:eval-robustness}). Because both are independent random partitions of the same compartments, the ranking split's held-out portion partially overlaps with the training data of most evaluation folds, and can by chance overlap with at least one fold's test data. This is a diffuse selection-leakage risk, not the same as training and testing on identical rows -- feature ranking could be mildly favoured by data a given fold later treats as held out. Full per-fold nested ranking would close this exactly, at five times the ranking compute, for a risk assessed as low in practice; it is disclosed here as a limitation rather than resolved.
...
Set~2
& Set~1 plus the ten highest-ranked candidates from the complete environmental pool. Candidates failing VIF screening are replaced by the next-ranked candidate. \\
...
The final sets are not strictly nested because VIF screening is repeated after expanding the candidate pool. For example, elevation appears in Sets~2 and~3 but is removed from Set~4 when correlated terrain and wind predictors are considered together. This indicates redundancy within Set~4, not that elevation is ecologically unimportant.

% Full per-question Set membership (RSQ1=Q3, RSQ2=Q1, RSQ3=Q2 -- verified against production
% code, not just target-name matching; see documentation/refocus_draft/working_results/
% appendix.tex, "Feature-set membership by research question" for the mapping evidence and
% the converted tables) moved to Appendix~\ref{app:feature-sets}, 2026-08-23.
...
% all -- checked both this file and the canonical 20th_1_31_pm_audited.tex. Revisit once
% Results is finalised: either define both terms here (natural home, since this is where
% check (4) itself is described) and let Results just use the acronyms, or confirm Results
% will keep defining them inline and leave Methodology as-is.
...
Global models assume one environment--growth relationship across the study area. Q2 tests whether allowing this relationship to vary by location improves explanation and reduces residual spatial clustering. We use GNNWR \cite{du2020Geographically_C6_GWR}: it personalises each plot's prediction based on where it physically sits, assuming nearby plots behave similarly, and lets its coefficients (how much each variable matters) drift smoothly across the map -- two plots right next to each other are treated as alike, just because they are close. A neural network learns this spatial weighting directly from geographic coordinates, without a fixed neighbourhood size chosen in advance.
