...
% AUDIT: this sentence and Table~\ref{tab:data-cleaning-funnel} describe the maturity cutoff (row 7) as applying uniformly to downstream modelling, but Q1's models (mean_cr_residual, EN/XGBoost/LMM) do not apply it -- they use the pre-filter, row-6 population (71,766 plots, 4-survey), not the post-filter 58,112. Q2 and Q3 do apply it. A direct check (Q1 Set4, EN/XGBoost) found this increases R2 by roughly 0.05 (EN: 0.358 to 0.406) to 0.07 (XGBoost: 0.395 to 0.460) -- Q1's reported headline numbers are therefore conservative relative to a maturity-filtered population, not overstated. Identified during review, not resolved given time constraints; see the matching note in the Q1 Results section.
...
\paragraph*{Does the physics loss weight itself help or hurt?}
\textbf{Raising the physics-loss weight made predictions worse every time; dropping it entirely closes most, but not all, of the DNN-PINN gap.} Testing $\lambda_{\mathrm{phys}} \in \{0,1,2\}$ on the unconstrained, pre-forward-pass-fix PINN-$k$, every increase past zero reduced accuracy, on both cohorts. Setting $\lambda_{\mathrm{phys}}=0$ raised $R^2$ by $0.05$ on 4-survey -- a real gain -- and by $0.010$ on 6-survey, within noise. The physics loss appears to actively fight the data-fit objective on shared network weights, rather than usefully regularising it. With $\lambda_{\mathrm{phys}}=0$, the 4-survey DNN-PINN gap shrinks from $0.080$ to $0.030$ (DNN $0.655$ vs.\ unconstrained PINN-$k$ $0.625$, 5-fold spatial-block mean, verified against \texttt{outputs/run\_logs/}), and PINN-$k$ edges past the DNN on 6-survey ($0.694$ vs.\ $0.684$). The remaining $0.030$ gap is unexplained -- plausibly PINN-$k$'s extra sub-network parameters, plausibly ordinary run-to-run variation between two structurally different architectures; not settled further here. These numbers use the unconstrained (pre-fix) architecture, not the corrected-forward-pass PINN-$k$ reported in Table~\ref{tab:results-q3} and the paragraph below -- the two are separate diagnostics, not the same run.
