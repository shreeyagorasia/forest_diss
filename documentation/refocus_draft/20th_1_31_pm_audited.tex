...
Q2 fits a separate height ceiling for each plot while fixing the regional Forestry Commission rate and shape values. $p_1$--$p_5$ are Forest Research's own regression coefficients, tabulated per species and region, that convert a General Yield Class number into a full Chapman--Richards curve: $p_4$ is the shared growth rate, $p_5$ is the shared curve-shape exponent, and $p_1$--$p_3$ combine (Equation~\eqref{eq:yldc-asymptote} below) to give the yield-class-implied height ceiling. This project did not derive these coefficients; it uses Forest Research's own values. For plot $i$ at survey $t$, let:
\begin{align}
...
$\Delta y_{\max,i}$ is a single value per plot, not per survey: one number summarising a plot's whole observed history against its yield-class expectation. A positive value means the plot's own fitted ceiling sits above what its assigned yield class predicts; a negative value means it sits below. $\hat y_{\max,i}$ is an estimated summary of the fitted curve, not the plot's observed maximum recorded height. Plots with short or narrow-age-range survey histories give a less reliable $\hat y_{\max,i}$, since the fit is anchored on fewer points.
...
\textbf{The 250 plots with enough data to test split into two mechanistically distinct groups: most (71.6\%) show the fixed-$k$ artifact signature and sit on exposed terrain; the rest (28.4\%) look like genuinely fast growth on sheltered terrain.}
