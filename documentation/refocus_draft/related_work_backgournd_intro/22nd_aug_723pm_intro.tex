...
% "Main Finding" filled in 2026-08-24: drawn from Q1/Q2/Q3 Discussion/Conclusion sections
% (results_q1_21_08_0754pm.tex, results_q2_22_08_0214.tex, results_q3_23_08_0540am.tex),
% restricted to vocabulary already established earlier in this chapter -- no new jargon from
% the Results chapters (no SHAP, Moran's I, R^2, compartment, permutation importance, etc.).
...
Across the three questions, environment consistently matters, but how it matters changes with the question asked. Environmental and management conditions are genuinely tied to a plot's persistent deviation from the shared growth curve, and the conditions identified are broadly consistent across Elastic Net, mixed-effects, and XGBoost models (Q1). Letting growth-environment links change by location, via GNNWR, does not pick up much that Q1's models miss: most of the unusual local results it surfaces turn out to be noise or artefacts of how the target itself was built, rather than real local effects (Q2). For predicting top height directly, a standard model -- gradient-boosted trees -- wins over both the deep neural network and the physics-informed variants tested; adding a Chapman-Richards constraint does not improve accuracy, but it does give more interpretable growth parameters (Q3). Taken together, environmental and management conditions explain real, persistent growth differences, and independent attribution methods agree on which conditions matter -- but neither letting those relationships vary by location nor building in growth-curve physics improves prediction of top height itself; interpretability, not accuracy, is what a physics-informed model buys.
