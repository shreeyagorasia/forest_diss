...
\item \textbf{RQ2b:} why NLME and Elastic Net disagree specifically on \texttt{topex}
  has a theoretically grounded, partially evidenced account (spatial confounding, since
  \texttt{topex} is 74\% between-compartment variance, a precondition confirmed
  directly), not a fully settled one: \texttt{slope\_degrees}, which stays stable, is
  also substantially between-compartment (63\%), so this alone does not cleanly
  separate the two variables. Report as partially explained, not fully open and not
  fully resolved.
\item \textbf{RQ2b:} the static-climatology time-mismatch explanation for the unstable
  ...
  instability is not explained by this or any other hypothesis so far.
...
\fig{\textbf{RQ1, Figure 1 -- paired per-fold R2, XGBoost vs.\ DNN.} Plots: XGBoost
(tuned) vs.\ DNN, Set3, both cohorts, all 5 spatial\_block\_kfold folds. Design: a
slopegraph, not a bar chart -- two columns ("DNN", "XGBoost"), one line per fold
connecting that fold's R2 in each model, two panels (one per cohort) so the 5/5 vs.\
4/5 win split is visible side by side; the one reversed fold (4survey, fold 0) marked
with a distinct colour/marker. Establishes: that XGBoost's advantage holds fold-by-fold,
not only on average -- the table's pooled mean$\pm$SD alone cannot distinguish "wins
consistently, small margin" from "wins on average, flips unpredictably." Answers RQ1
item 1. No other figure serves this purpose.}

\fig{\textbf{RQ1, Figure 2 -- the physics trade-off (accuracy vs.\ identifiability).}
Two panels. Panel A (accuracy cost): PINN and PINN\_k, both cohorts, R2 vs.\
physics/trajectory weight (0, 1, 2), point+error-bar (single-split CI or fold SD), one
line per model, cohort as facet or colour. Panel B (identifiability benefit): PINN\_k
only (the only variant with an independently learned $k$), the $y_{\max}/k$
correlation at each tested weight with its bootstrap CI as an error bar --
\textbf{scope Panel B to w=0/w=1 only, no confirmed w=2 correlation number exists, leave
that column empty rather than guess}. Establishes: the "accuracy down, identifiability
up" trade-off (RQ1 item 5 + RQ2a item 3) as one connected argument -- no table in this
chapter currently shows the weight-sweep numbers at all, this is the only place the two
halves of the trade-off appear together rather than in two separate paragraphs the
reader has to link themselves. Answers RQ1 item 5, bridges to RQ2a item 3.}
...
\fig{\textbf{RQ2a -- quartile reduction by model and cohort, with the permutation
control included.} Plots: DNN and XGBoost (env-conditioned, Set3), both cohorts, mean
reduction by CR-baseline-error quartile (Q1--Q4), \textbf{plus a third series: the
permuted-environment XGBoost control from
\texttt{TEMP\_rq2a\_permutation\_check\_2026-08-17.tex} -- already computed, no new
fitting needed}. Design: paired points with error bars (fold SD for XGBoost, 5-seed SD
for DNN -- note directly on the figure, not only the caption, that these are not on the
same basis), x-axis = quartile, y-axis = mean reduction (m), a horizontal zero line so
Q1's negative and Q4's positive values read immediately. Establishes: not just the
quartile split, but the actual mechanism -- real and permuted XGBoost overlap almost
exactly at Q1 and diverge visibly at Q4, showing the real-vs-flexibility split directly
rather than requiring the reader to trust the prose. Answers RQ2a item 1. No other
...
Panel B (spatial clustering doesn't move): Moran's I by set (Set1--Set4), EN and
XGBoost as two lines, deliberately near-flat -- the flatness is itself the finding.
...
point+CI, 4survey only. Panel B: residual Moran's I by set/model/cohort, paired so the
4survey decrease and 6survey increase sit side by side -- the independent
corroboration for Panel A's edge. Panel C: a spatial map of GNNWR's own per-plot local
