...
(4survey: $0.590$ vs.\ $0.584$; 6survey: $0.728$ vs.\ $0.731$). This is architectural, confirmed
directly in code (see the physics-weight ablation above): terrain never reaches PINN's trunk, so
there is nothing terrain-derived for a random split to leak between train and test, regardless of
$\lambda$. \texttt{dnn\_noenv} -- a plain DNN with no physics loss and, like PINN's trunk, no
terrain in its input -- confirms this is the reason: it shows the same small gap as PINN ($0.657$
vs.\ $0.633$ on 4survey), not the terrain-DNN's large one, even though its spatial-block fit
($0.633$) matches the terrain-DNN's ($0.634$) almost exactly. Matched fit quality with a very
different split-gap rules out both physics regularisation (\texttt{dnn\_noenv} has none) and a
weaker overall fit (the two fits are essentially equal) -- the presence of terrain in the trunk is
...
This is an architecture limit, not a physics-vs-data conflict. Terrain and wind features never
reach the main network's prediction path, at any $\lambda$ -- they only ever feed the physics-loss
target. Height predictions cannot vary with terrain, whatever the weight. Raising $\lambda$ just
pushes training harder toward a terrain-conditioned target the network cannot use per plot: more
noise, not more information. This is why removing the physics loss narrows most of the DNN-PINN
gap on 4survey (DNN $R^2=0.655$ vs.\ unconstrained PINN$_k$ $0.629$, down from a $0.080$ gap at
$\lambda=1$) and closes it entirely on 6survey, where the unconstrained model slightly beats the
DNN ($0.719$ vs.\ $0.684$).

Physics guidance does change the learned CR parameters, but this never reaches the predictions
above: the $y_{\max}$/$k$ sub-networks feed only the training loss, never the model's own forward
pass. So the correlation below describes training dynamics, not identifiability of the deployed
model. Without guidance ($\lambda=0$), $y_{\max}$ and $k$ collapse to two constants per fold, not
genuine per-plot values -- so their reported correlation ($r=-0.935$) is not a real measure of
...
but this does not fix identifiability, and neither correlation reflects what the model predicts.
...
competitive, but CR guidance did not improve predictive accuracy. This is architectural: terrain
never reaches PINN or PINN$_k$'s predictions, at any weight, only the training loss. Random plot
splitting gave an optimistic DNN estimate. The current $\lambda=0$ implementation does not support
a controlled claim about parameter identifiability.
...
The weak PINN result does not show that CR theory is unsuitable for forest growth. It shows an
architecture limit: terrain and wind features only ever reach PINN and PINN$_k$ through the
physics-loss target, never the height prediction itself, at any weight. Raising the weight cannot
help accuracy under this design -- it only pushes training harder toward a target the network
cannot use per plot. Whether a pooled CR derivative is also too rigid for locally varying
management, disturbance or site conditions remains untested; that needs a model where terrain
reaches the prediction directly, which neither PINN variant here does.
